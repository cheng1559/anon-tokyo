...
class ClosedLoopEnv:
    ...
    def _profile(self, name: str):
        return self.profiler.record(name) if self.profiler is not None else nullcontext()
    ...
    def reset(self, batch: dict[str, Any]) -> dict[str, Tensor | list[str]]:
        with self._profile("env.reset.to_device"):
    ...
    def step(self, actions: Tensor) -> tuple[dict[str, Tensor | list[str]], Tensor, Tensor, dict[str, Tensor]]:
        ...
        with self._profile("env.step.prepare"):
        ...
        with self._profile("env.step.dynamics"):
        ...
        with self._profile("env.step.replay_blend"):
        ...
        with self._profile("env.step.append_state"):
        ...
        with self._profile("env.step.reward"):
            reward_state = self._reward_state()
            reward, done_step, info, next_goal_reached = compute_rewards(
                reward_state,
                self.config.rewards,
                profiler=self.profiler,
            )
        with self._profile("env.step.done_update"):
    ...
    def get_obs(self) -> dict[str, Tensor | list[str]]:
        ...
        with self._profile("env.get_obs"):
